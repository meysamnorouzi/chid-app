/**
 * Basic Examples Page for Tabs
 */

import { useState } from 'react';
import { Tabs, CodeBlock } from '../../../components/shared';
import { useDocumentationTheme } from '../../../theme';
import { basicUsageCode, withIconsCode, withBadgesCode, withContentCode } from './codeExamples/codeExamples';

const BasicExamples = () => {
  const { theme } = useDocumentationTheme();
  const [value1, setValue1] = useState<string | number>('tab1');
  const [value2, setValue2] = useState<string | number>('home');
  const [value3, setValue3] = useState<string | number>('inbox');
  const [value4, setValue4] = useState<string | number>('tab1');

  // Icons
  const HomeIcon = (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
    </svg>
  );

  const SettingsIcon = (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
    </svg>
  );

  return (
    <div className="w-full mb-8">
      <header className="mb-8">
        <h1 
          className="text-4xl font-bold mb-4"
          style={{ color: theme.colors.light.inverse }}
        >
          Basic Examples
        </h1>
        <p 
          className="text-lg mb-4"
          style={{ color: theme.colors.gray[600] }}
        >
          Basic usage examples of the Tabs component.
        </p>
      </header>

      <section className="mb-12 w-full">
        <div className="space-y-8 w-full">
          {/* Basic Tabs */}
          <div className="w-full">
            <h3 
              className="text-lg font-medium mb-3"
              style={{ color: theme.mode === 'dark' ? theme.colors.light.inverse : theme.colors.gray[700] }}
            >
              Basic Tabs
            </h3>
            <div className="mb-4">
              <Tabs
                items={[
                  { value: 'tab1', label: 'Tab 1' },
                  { value: 'tab2', label: 'Tab 2' },
                  { value: 'tab3', label: 'Tab 3' },
                ]}
                value={value1}
                onChange={(val) => setValue1(val)}
              />
            </div>
            <CodeBlock
              code={basicUsageCode}
              title="Code"
              componentPath="@/components/shared/Tabs"
            />
          </div>

          {/* With Icons */}
          <div className="w-full">
            <h3 
              className="text-lg font-medium mb-3"
              style={{ color: theme.mode === 'dark' ? theme.colors.light.inverse : theme.colors.gray[700] }}
            >
              With Icons
            </h3>
            <div className="mb-4">
              <Tabs
                items={[
                  { value: 'home', label: 'Home', icon: HomeIcon },
                  { value: 'settings', label: 'Settings', icon: SettingsIcon },
                ]}
                value={value2}
                onChange={(val) => setValue2(val)}
              />
            </div>
            <CodeBlock
              code={withIconsCode}
              title="Code"
              componentPath="@/components/shared/Tabs"
            />
          </div>

          {/* With Badges */}
          <div className="w-full">
            <h3 
              className="text-lg font-medium mb-3"
              style={{ color: theme.mode === 'dark' ? theme.colors.light.inverse : theme.colors.gray[700] }}
            >
              With Badges
            </h3>
            <div className="mb-4">
              <Tabs
                items={[
                  { value: 'inbox', label: 'Inbox', badge: 5 },
                  { value: 'sent', label: 'Sent', badge: 12 },
                  { value: 'drafts', label: 'Drafts', badge: 3 },
                ]}
                value={value3}
                onChange={(val) => setValue3(val)}
              />
            </div>
            <CodeBlock
              code={withBadgesCode}
              title="Code"
              componentPath="@/components/shared/Tabs"
            />
          </div>

          {/* With Content */}
          <div>
            <h3 
              className="text-lg font-medium mb-3"
              style={{ color: theme.mode === 'dark' ? theme.colors.light.inverse : theme.colors.gray[700] }}
            >
              With Content
            </h3>
            <div className="mb-4">
              <Tabs
                items={[
                  { 
                    value: 'tab1', 
                    label: 'Tab 1', 
                    content: (
                      <div style={{ padding: '1rem', color: theme.colors.gray[700] }}>
                        This is the content for Tab 1
                      </div>
                    )
                  },
                  { 
                    value: 'tab2', 
                    label: 'Tab 2', 
                    content: (
                      <div style={{ padding: '1rem', color: theme.colors.gray[700] }}>
                        This is the content for Tab 2
                      </div>
                    )
                  },
                  { 
                    value: 'tab3', 
                    label: 'Tab 3', 
                    content: (
                      <div style={{ padding: '1rem', color: theme.colors.gray[700] }}>
                        This is the content for Tab 3
                      </div>
                    )
                  },
                ]}
                value={value4}
                onChange={(val) => setValue4(val)}
              />
            </div>
            <CodeBlock
              code={withContentCode}
              title="Code"
              componentPath="@/components/shared/Tabs"
            />
          </div>
        </div>
      </section>
    </div>
  );
};

export default BasicExamples;

